...
static std::atomic_bool stream_client_active(false);
static std::atomic<uint32_t> stream_frames(0);
static std::atomic<uint32_t> stream_send_failures(0);
...
static esp_err_t stream_handler(httpd_req_t *req) {
...
#else
  stream_client_active.store(true);
#endif
  ...
  // old frames queue up and increase latency.
  ...
  while (true) {
    ...
    if (res != ESP_OK) {
      stream_send_failures.fetch_add(1);
      log_e("Send frame failed");
      break;
    }
    int64_t fr_end = esp_timer_get_time();
    uint32_t frame_time = (uint32_t)((fr_end - last_frame) / 1000);
    last_frame = fr_end;
    stream_frames.fetch_add(1);
    stream_last_frame_ms.store(frame_time);
    stream_last_frame_bytes.store((uint32_t)_jpg_buf_len);
    const uint32_t instantFpsX10 = frame_time ? (10000U / frame_time) : 0;
    const uint32_t previousFpsX10 = stream_fps_x10.load();
    // A light EWMA keeps the overlay readable without hiding real changes.
    stream_fps_x10.store(previousFpsX10 ? ((previousFpsX10 * 7U + instantFpsX10 * 3U) / 10U) : instantFpsX10);
  }
...
static esp_err_t metrics_handler(httpd_req_t *req) {
  ...
  const int length = snprintf(
    json,
    sizeof(json),
    "{\"stream_active\":%s,\"ap_clients\":%u,\"frames\":%lu,\"send_failures\":%lu,"
    "\"last_frame_ms\":%lu,\"last_frame_bytes\":%lu,\"fps\":%.1f,"
    "\"free_heap\":%lu,\"min_free_heap\":%lu,\"free_psram\":%lu,\"uptime_ms\":%lu,"
    "\"processing_fps\":%.1f,\"last_processing_ms\":%.1f}\n",
    stream_client_active.load() ? "true" : "false",
    WiFi.softAPgetStationNum(),
    stream_frames.load(),
    stream_send_failures.load(),
    stream_last_frame_ms.load(),
    stream_last_frame_bytes.load(),
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  if (!strcmp(variable, "framesize")) {
    ...
    log_i("Unsupported command: %s", variable);
    res = -1;
  }
...
bool startCameraServer() {
  ...
  // httpd_config_t 的单位只能是整秒；流 socket 在 stream_handler 中另设
  // 400 ms 的精确超时。这里使用允许的最短整秒值作为兜底。
